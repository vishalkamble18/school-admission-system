...
export default function Home() {
  ...
  return (
    <div className="text-white relative">
      {/* ================= HERO ================= */}
      <section className="min-h-screen flex flex-col justify-center items-center bg-gradient-to-r from-blue-600 to-indigo-700 text-center px-4 relative">
          ...
          {owners.map((owner) => (
            ...
            >
              <img
                ...
                className="w-20 h-20 rounded-full object-cover border-2 border-white shadow-lg transition transform duration-300 hover:scale-110 hover:shadow-2xl cursor-pointer"
              />

              {/* Tooltip */}
              <div className="absolute left-1/2 -translate-x-1/2 top-24 bg-black/90 text-white text-xs px-3 py-1 rounded opacity-0 group-hover:opacity-100 transition whitespace-nowrap pointer-events-none">
        ...
        {selectedOwner && (
          ...
          >
            <motion.div
              ...
              className="bg-white text-gray-800 rounded-xl p-6 max-w-sm w-full text-center shadow-xl"
            >
              <img
                src={selectedOwner.image}
                alt={selectedOwner.name}
                className="w-28 h-28 rounded-full mx-auto object-cover shadow mb-4"
              />

              <h3 className="text-xl font-bold">{selectedOwner.name}</h3>
              <p className="text-blue-600 font-medium">{selectedOwner.role}</p>

              <button
                onClick={() => setSelectedOwner(null)}
                className="mt-5 bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition"
              >
                Close
              </button>
            </motion.div>
